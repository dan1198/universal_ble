...
std::unordered_map<std::string, winrt::event_token>
	characteristicsTokens{}; // TODO: Remove the map and store the token inside the
...
void UniversalBlePlugin::EnableBluetooth(std::function<void(ErrorOr<bool> reply)> result)
{
	...
	async_c.Completed([&, result](IAsyncOperation<RadioAccessStatus> const &sender,
...
std::optional<FlutterError> UniversalBlePlugin::StartScan(const UniversalScanFilter *filter)
{
	if (bluetoothRadio && bluetoothRadio.State() == RadioState::On) {
		...
		if (!bluetoothLEWatcher) {
			...
			(void)bluetoothLEWatcher.AllowExtendedAdvertisements(true);
			resetScanFilter();
				...
				if (hasCustomFilters) {
					...
					if (!filter->with_services().empty()) {
						for (const auto &uuid : filter->with_services()) {
							bluetoothLEWatcher.AdvertisementFilter()
								.Advertisement()
								.ServiceUuids()
								.Append(uuid_to_guid(
									std::get<std::string>(
			...
			bluetoothLEWatcherReceivedToken = bluetoothLEWatcher.Received(
				{this, &UniversalBlePlugin::BluetoothLEWatcher_Received});
		}
...
std::optional<FlutterError> UniversalBlePlugin::Disconnect(const std::string &device_id)
{
	...
	uiThreadHandler_.Post([deviceAddress] {
		callbackChannel->OnConnectionChanged(_mac_address_to_str(deviceAddress), false,
						     nullptr, SuccessCallback, ErrorCallback);
	});
...
void UniversalBlePlugin::SetNotifiable(
	...
	int64_t ble_input_property, std::function<void(std::optional<FlutterError> reply)> result)
{
	try {
		auto it = connectedDevices.find(_str_to_mac_address(device_id));
		if (it == connectedDevices.end()) {
		...
		GattCharacteristicObject &gatt_characteristic_holder =
			bluetoothAgent._fetch_characteristic(service, characteristic);
...
void UniversalBlePlugin::ReadValue(const std::string &device_id, const std::string &service,
				   ...
				   std::function<void(ErrorOr<std::vector<uint8_t>> reply)> result)
{
	try {
		...
		auto async_c = gattCharacteristic.ReadValueAsync(
			Devices::Bluetooth::BluetoothCacheMode::Uncached);
		async_c.Completed([&, result](IAsyncOperation<GattReadResult> const &sender,
					      AsyncStatus const args) {
...
void UniversalBlePlugin::WriteValue(const std::string &device_id, const std::string &service,
				    const std::string &characteristic,
				    const std::vector<uint8_t> &value, int64_t ble_output_property,
...
void UniversalBlePlugin::RequestMtu(const std::string &device_id, int64_t expected_mtu,
				    std::function<void(ErrorOr<int64_t> reply)> result)
{
	try {
		auto it = connectedDevices.find(_str_to_mac_address(device_id));
		...
		auto bluetoothAgent = *it->second;
		auto async_c =
			GattSession::FromDeviceIdAsync(bluetoothAgent.device.BluetoothDeviceId());
		async_c.Completed([&, result](IAsyncOperation<GattSession> const &sender,
					      AsyncStatus const args) {
...
void UniversalBlePlugin::IsPaired(const std::string &device_id,
...
{
	try {
		auto device = async_get(BluetoothLEDevice::FromBluetoothAddressAsync(
			_str_to_mac_address(device_id)));
		...
		if (unpairingStatus ==
		    Enumeration::DeviceUnpairingResultStatus::OperationAlreadyInProgress) {
...
winrt::fire_and_forget UniversalBlePlugin::InitializeAsync()
{
	...
	for (auto &&radio : radios) {
		if (radio.Kind() == RadioKind::Bluetooth) {
			...
			radioStateChangedRevoker = bluetoothRadio.StateChanged(
				winrt::auto_revoke,
				{this, &UniversalBlePlugin::Radio_StateChanged});
			break;
		}
	}
	if (!bluetoothRadio) {
...
UniversalBlePlugin::PairAsync(std::string device_id,
			      std::function<void(ErrorOr<bool> reply)> result)
{
	try {
		...
		if (deviceInformation.Pairing().IsPaired()) {
			...
			bool isPaired = pairResult.Status() ==
					Enumeration::DevicePairingResultStatus::Paired;
			result(isPaired);
			std::string errorStr = parsePairingFailError(pairResult);
			uiThreadHandler_.Post([device_id, isPaired, errorStr] {
				callbackChannel->OnPairStateChange(device_id, isPaired, &errorStr,
...
winrt::fire_and_forget
UniversalBlePlugin::CustomPairAsync(std::string device_id,
				    std::function<void(ErrorOr<bool> reply)> result)
{
	try {
		auto device = co_await BluetoothLEDevice::FromBluetoothAddressAsync(
		...
		if (deviceInformation.Pairing().IsPaired()) {
			...
			winrt::event_token token = customPairing.PairingRequested(
			...
			DevicePairingProtectionLevel protectionLevel =
				deviceInformation.Pairing().ProtectionLevel();
...
void UniversalBlePlugin::PairingRequestedHandler(DeviceInformationCustomPairing sender,
						 ...
						 bool isConnectable)
{
	...
	if (it != scanResults.end()) {
		...
		if (scanResult.name() != nullptr && !scanResult.name()->empty() &&
		    currentScanResult.name() != nullptr && !currentScanResult.name()->empty()) {
		...
		if ((scanResult.manufacturer_data_list() == nullptr ||
		     scanResult.manufacturer_data_list()->empty()) &&
		    currentScanResult.manufacturer_data_list() != nullptr) {
			scanResult.set_manufacturer_data_list(
				currentScanResult.manufacturer_data_list());
			shouldUpdate = true;
		}
...
void UniversalBlePlugin::setupDeviceWatcher()
{
	...
	deviceWatcherAddedToken =
		deviceWatcher.Added([this](DeviceWatcher sender, DeviceInformation deviceInfo) {
	...
	deviceWatcherRemovedToken =
		deviceWatcher.Removed([this](DeviceWatcher sender, DeviceInformationUpdate args) {
...
void UniversalBlePlugin::onDeviceInfoReceived(DeviceInformation deviceInfo)
{
	...
	if (!(properties.HasKey(isConnectableKey) &&
	      (properties.Lookup(isConnectableKey).as<IPropertyValue>()).GetBoolean()) ||
	    !properties.HasKey(deviceAddressKey)) {
		return;
	}

	auto bluetoothAddressPropertyValue =
		properties.Lookup(deviceAddressKey).as<IPropertyValue>();
	...
	if (scanResults.count(deviceAddress) > 0) {
		...
		if (properties.HasKey(isPairedKey)) {
			auto isPairedPropertyValue =
				properties.Lookup(isPairedKey).as<IPropertyValue>();
		...
		if (properties.HasKey(signalStrengthKey)) {
			auto rssiPropertyValue =
				properties.Lookup(signalStrengthKey).as<IPropertyValue>();
...
void UniversalBlePlugin::BluetoothLEWatcher_Received(BluetoothLEAdvertisementWatcher sender,
						     BluetoothLEAdvertisementReceivedEventArgs args)
{
	try {
			...
			for (BluetoothLEManufacturerData msd :
				...
				UniversalManufacturerData universalManufacturerData =
					UniversalManufacturerData(
						static_cast<int64_t>(msd.CompanyId()),
						to_bytevc(msd.Data()));
				manufacturerDataEncodableList.push_back(
					flutter::CustomEncodableValue(universalManufacturerData));
			}
		}
		...
		if (!manufacturerDataEncodableList.empty()) {
			universalScanResult.set_manufacturer_data_list(
				manufacturerDataEncodableList);
		}
		...
		if (it != deviceWatcherDevices.end()) {
			...
			bool isPaired = deviceInfo.Pairing().IsPaired();
			if (properties.HasKey(isPairedKey)) {
				isPaired = (properties.Lookup(isPairedKey).as<IPropertyValue>())
						   .GetBoolean();
			}
			universalScanResult.set_is_paired(isPaired);
...
void UniversalBlePlugin::Radio_StateChanged(Radio radio, IInspectable args)
{
	...
	uiThreadHandler_.Post([state] {
		callbackChannel->OnAvailabilityChanged(static_cast<int>(state), SuccessCallback,
						       ErrorCallback);
	});
}
...
winrt::fire_and_forget
UniversalBlePlugin::WriteAsync(GattCharacteristic characteristic, GattWriteOption writeOption,
			       const std::vector<uint8_t> &value,
			       std::function<void(std::optional<FlutterError> reply)> result)
{
	try {
		auto writeResult =
			co_await characteristic.WriteValueAsync(from_bytevc(value), writeOption);
		switch (writeResult) {
		case GenericAttributeProfile::GattCommunicationStatus::Success:
			result(std::nullopt);
			co_return;
		...
		std::cout << "WriteValueLog: " << winrt::to_string(err.message())
			  << " ErrorCode: " << std::to_string(errorCode) << std::endl;
...
winrt::fire_and_forget UniversalBlePlugin::ConnectAsync(uint64_t bluetoothAddress)
{
	...
	if (!device) {
		...
		uiThreadHandler_.Post([bluetoothAddress] {
			callbackChannel->OnConnectionChanged(
				_mac_address_to_str(bluetoothAddress), false,
	...
	if (status != GattCommunicationStatus::Success) {
		...
		uiThreadHandler_.Post([bluetoothAddress, error] {
			callbackChannel->OnConnectionChanged(_mac_address_to_str(bluetoothAddress),
							     false, &error, SuccessCallback,
							     ErrorCallback);
		});
	...
	winrt::event_token connnectionStatusChangedToken = device.ConnectionStatusChanged(
		{this, &UniversalBlePlugin::BluetoothLEDevice_ConnectionStatusChanged});
	auto deviceAgent = std::make_unique<BluetoothDeviceAgent>(
		device, connnectionStatusChangedToken, gatt_map_);
	...
	uiThreadHandler_.Post([bluetoothAddress] {
		callbackChannel->OnConnectionChanged(_mac_address_to_str(bluetoothAddress), true,
						     nullptr, SuccessCallback, ErrorCallback);
	});
}

void UniversalBlePlugin::BluetoothLEDevice_ConnectionStatusChanged(BluetoothLEDevice sender,
								   IInspectable args)
{
		...
		for (auto &servicePair : deviceAgent->gatt_map_) {
			...
			for (auto &characteristicPair : service.characteristics) {
				GattCharacteristicObject &characteristic =
					characteristicPair.second;
				auto gattCharacteristic = characteristic.obj;
				std::stringstream uniqTokenKeyStream;
				uniqTokenKeyStream
					<< to_uuidstr(gattCharacteristic.Uuid())
					<< _mac_address_to_str(gattCharacteristic.Service()
								       .Device()
								       .BluetoothAddress());
				...
				if (characteristicsTokens.count(uniqTokenKey) != 0) {
					characteristic.obj.ValueChanged(
						characteristicsTokens[uniqTokenKey]);
	...
	std::function<void(ErrorOr<flutter::EncodableList> reply)> result)
{
	try {
		...
		Enumeration::DeviceInformationCollection devices =
			co_await Enumeration::DeviceInformation::FindAllAsync(selector);
		flutter::EncodableList results = flutter::EncodableList();
		for (auto &&deviceInfo : devices) {
			try {
				BluetoothLEDevice device =
					co_await BluetoothLEDevice::FromIdAsync(deviceInfo.Id());
					...
					if (serviceResult.Status() ==
						...
						for (auto service : serviceResult.Services()) {
							std::string serviceUUID =
								to_uuidstr(service.Uuid());
							if (std::find(with_services.begin(),
								      with_services.end(),
								      serviceUUID) !=
							    with_services.end()) {
								hasService = true;
				...
				results.push_back(
					flutter::CustomEncodableValue(universalScanResult));
			} catch (...) {
			}
		}
		result(results);
		...
		int errorCode = err.code();
		std::cout << "GetConnectedDeviceLog: " << winrt::to_string(err.message())
			  << " ErrorCode: " << std::to_string(errorCode) << std::endl;
		result(FlutterError(std::to_string(errorCode), winrt::to_string(err.message())));
	} catch (...) {
	...
	try {
		...
		for (auto &service : bluetoothDeviceAgent.gatt_map_) {
			...
			for (auto characteristicsMap : service.second.characteristics) {
				...
				if ((propertiesValue & GattCharacteristicProperties::Broadcast) !=
				    GattCharacteristicProperties::None) {
					properties.push_back(static_cast<int>(
						CharacteristicProperty::broadcast));
				}
				if ((propertiesValue & GattCharacteristicProperties::Read) !=
				    GattCharacteristicProperties::None) {
					properties.push_back(
						static_cast<int>(CharacteristicProperty::read));
				}
				if ((propertiesValue & GattCharacteristicProperties::Write) !=
				    GattCharacteristicProperties::None) {
					properties.push_back(
						static_cast<int>(CharacteristicProperty::write));
				}
				if ((propertiesValue &
				...
				if ((propertiesValue & GattCharacteristicProperties::Notify) !=
				    GattCharacteristicProperties::None) {
					properties.push_back(
						static_cast<int>(CharacteristicProperty::notify));
				}
				if ((propertiesValue & GattCharacteristicProperties::Indicate) !=
				    GattCharacteristicProperties::None) {
					properties.push_back(
						static_cast<int>(CharacteristicProperty::indicate));
				}
				if ((propertiesValue &
				...
				universalCharacteristics.push_back(
					flutter::CustomEncodableValue(UniversalBleCharacteristic(
						to_uuidstr(c.Uuid()), properties)));
			}

			auto universalBleService =
				UniversalBleService(to_uuidstr(service.second.obj.Uuid()));
			universalBleService.set_characteristics(universalCharacteristics);
			universalServices.push_back(
				flutter::CustomEncodableValue(universalBleService));
		}
...
winrt::fire_and_forget
UniversalBlePlugin::IsPairedAsync(std::string device_id,
				  std::function<void(ErrorOr<bool> reply)> result)
{
	try {
		auto device = co_await BluetoothLEDevice::FromBluetoothAddressAsync(
...
winrt::fire_and_forget UniversalBlePlugin::SetNotifiableAsync(
	BluetoothDeviceAgent &bluetoothDeviceAgent, const std::string &service,
	const std::string &characteristic,
	GattClientCharacteristicConfigurationDescriptorValue descriptorValue,
	std::function<void(std::optional<FlutterError> reply)> result)
{
	GattCharacteristicObject &gatt_characteristic_holder =
		bluetoothDeviceAgent._fetch_characteristic(service, characteristic);
	GattCharacteristic gattCharacteristic = gatt_characteristic_holder.obj;

	auto uuid = to_uuidstr(gattCharacteristic.Uuid());
	// Write to the descriptor.
	try {
		auto status = co_await gattCharacteristic
		...
		case GattCommunicationStatus::Success:
			break;
		case GattCommunicationStatus::Unreachable:
			std::cout << "FailedToSubscribe: Unreachable"
				  << to_uuidstr(gattCharacteristic.Uuid()) << std::endl;
			break;
		case GattCommunicationStatus::ProtocolError:
			std::cout << "FailedToSubscribe: ProtocolError"
				  << to_uuidstr(gattCharacteristic.Uuid()) << std::endl;
			break;
		case GattCommunicationStatus::AccessDenied:
			std::cout << "FailedToSubscribe: AccessDenied"
				  << to_uuidstr(gattCharacteristic.Uuid()) << std::endl;
			break;
		default:
			std::cout << "FailedToSubscribe: Unknown"
				  << to_uuidstr(gattCharacteristic.Uuid()) << std::endl;
		}
		if (status != GattCommunicationStatus::Success) {
		...
		std::cout << "FailedToPerformThisOperationOn: "
			  << to_uuidstr(gattCharacteristic.Uuid()) << std::endl;
		result(FlutterError("Failed to update notification state"));
		co_return;
	}
	...
	if (descriptorValue == GattClientCharacteristicConfigurationDescriptorValue::None) {
		if (characteristicsTokens.count(uniqTokenKey) != 0) {
			...
			std::cout << "Unsubscribed " << to_uuidstr(gattCharacteristic.Uuid())
				  << std::endl;
		}
	} else {
		// If a notification for the given characteristic is already in progress, swap the
		// callbacks.
		if (characteristicsTokens.count(uniqTokenKey) != 0) {
			std::cout << "A notification for the given characteristic is already in "
				     "progress. Swapping callbacks."
				  << std::endl;
			gattCharacteristic.ValueChanged(characteristicsTokens[uniqTokenKey]);
			characteristicsTokens[uniqTokenKey] = {0};
		}
		characteristicsTokens[uniqTokenKey] = gattCharacteristic.ValueChanged(
			{this, &UniversalBlePlugin::GattCharacteristic_ValueChanged});
	}
	result(std::nullopt);
}

void UniversalBlePlugin::GattCharacteristic_ValueChanged(GattCharacteristic sender,
							 GattValueChangedEventArgs args)
{
	...
	uiThreadHandler_.Post([sender, uuid, bytes] {
		callbackChannel->OnValueChanged(
			_mac_address_to_str(sender.Service().Device().BluetoothAddress()), uuid,
